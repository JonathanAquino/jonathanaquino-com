...
if (empty($data)){echo "Error retrieving data.";} //Return error message if scrape is unsuccessful.

else if (preg_match_between('<h1>','<\/h1>',$data)=="man ".$_REQUEST['cmd']){echo "Command does not exist.";} //Return error message if command doesn't exist.
    ...
    $cmdcreated = array("date"=>$temp_ay[0],"time"=>$temp_ay[1]);    //Date/time created.
    $temp_ay = explode(" ",preg_match_between('-\nLast used ','\n<\/div>',$data));
    $cmdlastused = array("date"=>$temp_ay[0],"time"=>$temp_ay[1]); //Date/time last used.
